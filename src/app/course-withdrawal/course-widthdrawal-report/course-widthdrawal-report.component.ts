  ...
  getSemReport(selectedSem:string){
    ...
    for(this.stud of this.students){
      if(this.stud.sem == selectedSem){
        ...
        for(this.c=0; this.c < this.stud.selectedCourseToWithdraw.length; this.c++){
          this.courseCode.add(this.stud.selectedCourseToWithdraw[this.c].code)
          this.courseName.add(this.stud.selectedCourseToWithdraw[this.c].name)
        }
      }
      this.i++
    }
